...
export class WAMonitoringService {
  constructor(
    ...
    (this as any).providerSession = Object.freeze(
      configService.get<ProviderSession>('PROVIDER')
    );
  }
    ...
    if (typeof time === 'number' && time > 0) {
      ...
      this.delInstanceTimeouts[instance] = setTimeout(
        async () => {
          try {
            if (
              this.waInstances[instance]?.connectionStatus?.state !== 'open'
            ) {
              if (
                this.waInstances[instance]?.connectionStatus?.state ===
                'connecting'
              ) {
                if (
                  (await this.waInstances[instance].integration) ===
  ...
  public async instanceInfo(instanceNames?: string[]): Promise<any> {
    if (instanceNames && instanceNames.length > 0) {
      const inexistentInstances = instanceNames
        ? instanceNames.filter((instance) => !this.waInstances[instance])
        : [];
  ...
  public async instanceInfoById(instanceId?: string, number?: string) {
    ...
    if (instanceId) {
      instanceName = await this.prismaRepository.instance
        .findFirst({ where: { id: instanceId } })
        .then((r) => r?.name);
      if (!instanceName) {
  ...
  public async cleaningUp(instanceName: string) {
    ...
    if (this.db.SAVE_DATA.INSTANCE) {
      ...
      if (findInstance) {
        ...
        rmSync(join(INSTANCE_DIR, instance.id), {
          recursive: true,
          force: true,
        });
  ...
  public async cleaningStoreData(instanceName: string) {
    ...
    await this.prismaRepository.webhook.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.chatwoot.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.proxy.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.rabbitmq.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.nats.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.sqs.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.integrationSession.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.typebot.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.websocket.deleteMany({
      where: { instanceId: instance.id },
    });
    await this.prismaRepository.setting.deleteMany({
      where: { instanceId: instance.id },
    });
  ...
  public async saveInstance(data: any) {
    try {
      const clientName =
        await this.configService.get<Database>('DATABASE').CONNECTION
          .CLIENT_NAME;
  ...
  private async loadInstancesFromDatabasePostgres() {
    const clientName =
      await this.configService.get<Database>('DATABASE').CONNECTION.CLIENT_NAME;
  ...
  private removeInstance() {
    this.eventEmitter.on('remove.instance', async (instanceName: string) => {
      try {
        await this.waInstances[instanceName]?.sendDataWebhook(
          Events.REMOVE_INSTANCE,
          null
        );
